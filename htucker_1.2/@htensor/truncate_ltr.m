...
for ii=find(ht_is_leaf)
  ...
  x_mat = matricize(x, ht_dims{ii}, [1:ht_dims{ii}-1, ht_dims{ii}+1:ndims(x)], false);
...
% [k(ii)]=Choose_R_U(sv{ii},opts);
%        R=min(size(x_mat));
%        k(ii)=opts.max_rank;
%        if (opts.max_rank<R)
%        dR = sv{ii}(opts.min_rank:end);
%         drops = dR(1:end-1)./dR(2:end);
%         [dmx,imx] = max(drops);
%          rel_drp = (k(ii)-opts.min_rank)*dmx/(sum(drops)-dmx);
%       if rel_drp>10
%             k(ii) = imx+opts.min_rank-1;
%          else
%              k(ii)=opts.max_rank;
%       end
%        end
