function [X,iter,relChgXPath,EF1,EF2,EF3] = Smoothlowrank_TVis( x0, known, rho,T,weight)
%-----------------------------------------------------------------------------------------------------
...
%     temp_Z=X+v1/beta(1);
    ...
    Z=full(Z);
     %- Y subproblem
     temp_y=beta(2)*Z+v2;
     diffT_L = diffT3_2( beta(3)*L - v3, sizeD,weight );
     numer1   = reshape( diffT_L + temp_y(:), sizeD);
    Y = real( ifftn( fftn(numer1) ./ (beta(3)*denom1 + beta(2)) ) );
    
    % -L subproblem
    diff_y = diff3_2(Y, sizeD,weight); 
    L      = softThres21( diff_y + v3/beta(3), rho/beta(3),N);  
%      L = SB_ATV(diff_y + v3/beta(3), rho/beta(3));
    %-X subproblem
       
    X= Z-v1/beta(1);
    X(known)=T(known);

%     L=prox_tv3d(diff_x + v2/beta(2), rho/beta(2));
